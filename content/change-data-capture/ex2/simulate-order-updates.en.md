...
::alert[_The order of updates to the Orders table is not preserved by Kinesis Data streams when changes are sent to the create order history lambda function. Hence if the order in which changes occur on the Orders table is important for your use case, DynamoDB Streams would be the preferred method to record changes from the Orders to the OrdersHistory table._]
